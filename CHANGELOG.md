...
- **CoolStats peut remplacer le tableau de bord de PrestaShop.** Une option de l'onglet Apparence installe le dashboard complet — graphiques, filtres et personnalisation compris — sur la page d'accueil du back-office, à la place du calendrier, des prévisions, de l'activité et de la colonne marketplace. Les blocs natifs ne sont pas laissés à côté : leur calendrier et les filtres de CoolStats donneraient deux périodes concurrentes, donc deux chiffres pour la même boutique. Désactivée par défaut, et la décocher rétablit le tableau de bord d'origine. La page d'accueil charge alors toutes les sections du dashboard à chaque visite, ce qui peut être long sur un gros catalogue.
- **La bande d'indicateurs du tableau de bord peut être masquée.** Une option la retire pour qui ne garde que le tableau de bord natif. Elle reste affichée par défaut, y compris sur les boutiques mises à jour, et s'efface d'elle-même quand CoolStats remplace le tableau de bord, où le dashboard porte déjà ces chiffres.
- **Le top produits se détaille par déclinaison.** Un interrupteur « Par déclinaison » dans l'entête de la section, à côté de Volume / CA, remplace la ligne par produit par une ligne par combinaison vendue, avec sa propre référence et son propre code-barres. La vue par produit reste celle par défaut, et l'export CSV suit le mode affiché.

### Corrigé

- **La référence et le code-barres du top produits désignaient une déclinaison prise au hasard.** PrestaShop enregistre dans la ligne de commande la référence de la déclinaison vendue : sur un produit décliné, le tableau en retenait une arbitrairement et l'affichait en face d'une quantité qui cumulait toutes les autres. La vue par produit montre désormais la référence et le code-barres du produit, et la vue par déclinaison ceux de la combinaison.
